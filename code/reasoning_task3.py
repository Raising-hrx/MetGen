...
def search_task3(data_item,reasoning_module_tuple,reasoning_module_abd_tuple, controller_tuple,\
    ...
    def check_state_too_long(state):
        ...
        if word_num > 400:
            ...
            new_state['fact_sents'] = new_state['fact_sents'][:-10]
            new_state['fact_ids'] = new_state['fact_ids'][:-10]
            return new_state
        else:
            return state
...
def run(args):
    ...
    print("loading data")
    with open(args.task3_data_file,'r') as f:
        datas = [json.loads(line) for line in f.readlines()]
    with open(args.task1_gold_data_file,'r') as f:
        gold_datas = [json.loads(line) for line in f.readlines()]
    ...
    for i, (data_item, gold_data_item) in enumerate(zip(eval_datas,gold_datas)):
        # if i < 240: continue
        gold_tree = get_gt_node_list(gold_data_item)
        pred_tree, all_pred_tree = search_task3(data_item,reasoning_module_tuple,reasoning_module_abd_tuple,controller_tuple,\
    ...
    save_result_path = osp.join(save_dir, f'{args.model_name_controller}.{timestr}.task3.prediction.json')

    # save args
    with open(save_result_path.replace('json','args.json'),'w') as f:
    ...
    with open(save_result_path.replace('json','details.json'),'w') as f:
        for gt,pt in zip(gold_trees,pred_trees):
            f.write(json.dumps({
                'gold_tree':gt,
                'pred_tree':pt,
            })+'\n')
    ...
    # task3 csv process
    # map task3 predictions to task1
    processed_results_tsv = []
    for pred_csv, input_item, gold_item in zip(results_tsv, eval_datas, gold_datas):
        gold_leaf_sent2id = {sent:sent_id for sent_id,sent in gold_item['meta']['triples'].items()}
        gold_leaf_sent2id

        id_map = {}
        for task3_id, sent in input_item['meta']['triples'].items():
            if sent in gold_leaf_sent2id.keys():
                id_map[task3_id] = gold_leaf_sent2id[sent]
            else:
                id_map[task3_id] = f"sent_distracter{len(id_map)}"
        # id_map: { 'sent14': 'sent2', 'sent15': 'sent_distracter14',}
        for task3_id, task1_id in id_map.items():
            pred_csv = pred_csv.replace(task3_id+" ", task1_id+" ")

        processed_results_tsv.append(pred_csv)

    with open(save_result_path.replace('.json','.processed.csv'),'w', newline='') as f:
        tsv_w = csv.writer(f, delimiter='\t')
        for r in processed_results_tsv:
            tsv_w.writerow([r])  
...
def get_params():
    ...
    # dateset
    parser.add_argument("--task3_data_file", type=str, default='')  
    parser.add_argument("--task1_gold_data_file", type=str, default='')   
    ...
    parser.add_argument("--reasoning_method", type=str, default="") 
    ...
    parser.add_argument("--fact_score_thre", type=float, default=0.7) 
    ...
    parser.add_argument("--abd_depth", type=int, default=0) 
