...
def search_task1(data_item,reasoning_module_tuple,reasoning_module_abd_tuple, controller_tuple,\
                 show=False, bleurt_scorer=None,
                 beam_num=1,abd_depth=0, 
...
#         return pred_tree, all_pred_tree
    ...
    # for task 1 we know that reasoning_step_num = face_num-1
    for beam_search_step in range(len(Sents)-1):
    ...
    final_states = sorted(beam_states[-1],key=lambda x:x['state_score'],reverse=True)
...
def run(args):
    ...
    for data_item in eval_datas:
        gold_tree = get_gt_node_list(data_item)
        pred_tree = search_task1(data_item,reasoning_module_tuple,reasoning_module_abd_tuple,controller_tuple,\
